...
config_file=/opt/gta-ai/config/finetune.env
. "$config_file"
...
mapfile -t job < <(
    /opt/gta-ai/app/.venv/bin/python - "$TRAIN_PENDING_MANIFEST" "$TRAIN_DATASET_PATH" <<'PY'
import hashlib
import json
import re
import sys
...
if [ -f "$completion_file" ]; then
    adapter_path=$(
        /opt/gta-ai/app/.venv/bin/python - "$completion_file" <<'PY'
import json
import sys
...
if [ -z "$adapter_path" ] || [ ! -f "$adapter_path/adapter_config.json" ]; then
    ...
    test -n "$adapter_path"
    /opt/gta-ai/app/.venv/bin/python - "$completion_file" "$adapter_path" "$revision" <<'PY'
import json
import os
import sys
from pathlib import Path
