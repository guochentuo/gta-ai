...
def classify(
    ...
    with _inference_lock, torch.inference_mode():
        for begin in range(0, len(waveform), samples_per_window):
            ...
            events = []
            for score, label_id in zip(top.values.tolist(), top.indices.tolist()):
            ...
            windows.append(
                {
                    ...
                    "endSec": round(min(len(waveform), begin + samples_per_window) / sample_rate, 3),
                    "events": events,
                }
            )
    return {
...
def main() -> None:
    ...
    port = int(os.getenv("AUDIO_EVENT_PORT", "18085"))
