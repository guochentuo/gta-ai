...
readonly endpoint="${1:-http://192.168.80.2}"
readonly image_path="${2:?用法：verify-image-services.sh ENDPOINT IMAGE_PATH}"

curl --fail --silent --show-error --max-time 10 "$endpoint:7102/health" | jq -e '.status == "ok"' >/dev/null
curl --fail --silent --show-error --max-time 10 "$endpoint:7103/health" | jq -e '.status == "ok"' >/dev/null

ocr_response="$(curl --fail --silent --show-error --max-time 180 \
  "$endpoint:7102/v1/ocr" -F "file=@$image_path")"
jq -e '.source_sha256 | length == 64' <<<"$ocr_response" >/dev/null
ocr_batch_response="$(curl --fail --silent --show-error --max-time 180 \
  "$endpoint:7102/v1/ocr-batch" \
...
image_response="$(curl --fail --silent --show-error --max-time 180 \
  "$endpoint:7103/v1/image-embeddings" -F "files=@$image_path")"
text_response="$(curl --fail --silent --show-error --max-time 180 \
  "$endpoint:7103/v1/text-embeddings" \
