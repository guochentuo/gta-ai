...
from gta_ai.idle_trainer import IdleTrainer, TrainerConfig, _write_json
...
def test_preemption_terminates_whole_training_process_group(tmp_path: Path, monkeypatch) -> None:
    ...
    killpg = Mock()
    cleanup = Mock()
    monkeypatch.setattr("gta_ai.idle_trainer.os.killpg", killpg)
...
def test_container_cleanup_force_removes_each_container_cgroup(
    ...
    trainer = IdleTrainer(config(tmp_path))
    run = Mock()
    monkeypatch.setattr("gta_ai.idle_trainer.subprocess.run", run)
