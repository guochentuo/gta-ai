...
parent_revision=$1
parent_completion=$app_root/data/training/runtime/$parent_revision/training-complete.json
dataset=$app_root/data/training/datasets/identity-correction.jsonl
manifest=$app_root/data/training/datasets/identity-correction.manifest.json
...
"$app_root/.venv/bin/python" "$app_root/training/identity/build_correction_dataset.py" \
    --facts "$facts" \
    --output "$dataset" \
    --manifest "$manifest"
